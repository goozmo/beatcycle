 ...
 */
 class Rebelmouse_Widget extends WP_Widget {
	...
	private	$hide_also_in_rm = "1";
	private	$hide_join_rm = "1";
	...
	public function widget( $args, $instance ) {
        ...
        if ( $instance['hide_also_in_rm'] == "1")
              $skip_array[] = "also-on-rm";
        if ( $instance['hide_join_rm'] == "1")
              $skip_array[] = "join-rm";
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['hide_also_in_rm'] = (bool)$new_instance['hide_also_in_rm'];
		$instance['hide_join_rm'] = (bool)$new_instance['hide_join_rm'];
	...
	function form( $instance ) {
		...
		$defaults = array(
                ...
                'hide_also_in_rm' => $this->hide_also_in_rm,
                'hide_join_rm' => $this->hide_join_rm,
		...
		<div class="rm-widget-content">
            ...
            <p>
                ...
                <div id="<?php echo $this->get_field_id( 'explanation' );?>" class="rm-power-site-explanation" <?php if ( $instance['features'] === "true" ) { ?>style="display:none;"<?php }?>>
                    ...
                    <a class="rm-power-site" href="https://www.rebelmouse.com/core/users/pricing/">Power Your Site</a>
                </div>
            </p>
            ...
            <p id="<?php echo $this->get_field_id('features-controls');?>" class="featured-controls<?php if ( $instance['features'] !== "true" ) { ?> inactive<?php }?>">
                ...
                <!-- Hide Also in RebelMouse: Checkbox -->
                <input type="checkbox" value="1" 
                        id="<?php echo $this->get_field_id( 'hide_also_in_rm' ); ?>" 
                        name="<?php echo $this->get_field_name( 'hide_also_in_rm' ); ?>" 
                        <?php if ( $instance['features'] !== "true" ) { ?>disabled="disabled"<?php } ?>
                        <?php echo ( $instance['hide_also_in_rm'] ? "checked='checked'" : ""); ?> />
                <label for="<?php echo $this->get_field_id( 'hide_also_in_rm' ); ?>"><?php _e('Hide Featured', 'framework') ?>.</label>
                <br>

                <!-- Hide Join us on RebelMouse: Checkbox -->
                <input type="checkbox" value="1"
                        id="<?php echo $this->get_field_id( 'hide_join_rm' ); ?>" 
                        name="<?php echo $this->get_field_name( 'hide_join_rm' ); ?>" 
                        <?php if ( $instance['features'] !== "true" ) { ?>disabled="disabled"<?php } ?>
                        <?php echo ( $instance['hide_join_rm'] ? "checked='checked'" : ""); ?> />
                <label for="<?php echo $this->get_field_id( 'hide_join_rm' ); ?>"><?php _e('Hide Join Us on RebelMouse Promo', 'framework') ?>.</label>
                <br>
