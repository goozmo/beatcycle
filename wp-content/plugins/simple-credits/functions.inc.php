...
function addScripts()
{
	...
	wp_enqueue_style( 'boot-scrap',  plugins_url('/css/alertify.bootstrap.css', __FILE__));
	wp_enqueue_style( 'boot-core',  plugins_url('/css/alertify.core.css', __FILE__));
	wp_enqueue_style( 'boot0-default',  plugins_url('/css/alertify.default.css', __FILE__));
	
	$params = array('error'=>$errorMessage, 'homeurl'=>get_home_url());
...
function creditwoocommerceshortcode()
{
	...
	while ( $loop->have_posts() )
	{
		...
		if (is_user_logged_in()) {
			$output .="<td><input class='btn btn-success' type='button' value='".$translate->wooTranslate('Buy Credits', get_bloginfo('language'))."' onclick='document.getElementById(\"add_to_credit".$counter."\").submit();'><form name='add_to_credit".$counter."' id='add_to_credit".$counter."' method='post'><input type='hidden' name='credit_id' value='".$loop->post->ID."'></form> </td>";
		} else {
			$loginMessage = $translate->wooTranslate('withoutloginMessage', get_bloginfo('language'));
...
/**
 * This function is used to add credit bundle to shopping cart
 */
