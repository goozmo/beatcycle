...
jQuery("#login-click").click(function() {
  jQuery("#login-toggle").animate({ opacity: 1.0 },200).slideToggle(100, function() {
    jQuery("#login-click").text(jQuery(this).is(':visible') ? "Close" : "Log In");
  });
});
</script>

<script>
jQuery('a.ptp-button').click(function(){
	var id = jQuery(this);
	var post_id = id.attr('rel');
	//alert("test" + post_id);
	jQuery('#add_credit_input').val(post_id);
	jQuery('#add_credit_form').submit();
});
</script>
...
<div id="hide-form-credit" style="display:none">
<form name="add_credit" id="add_credit_form" method="post">
	<input type="hidden" name="credit_id" value="" id="add_credit_input">
	</form>
</div>
