...
if (!$product->is_in_stock()) {
    echo '<a href="'.get_permalink().'" rel="nofollow" class="outstock">Out of Stock</a>';
} 
else
{ 
...
function woocommerce_variable_add_to_cart() {
		...
		?>
			<li class="center border-box-fix bike-number-<?php echo implode('', $value['attributes']); ?>">	<button type="button" name="bike" class="single_add_to_cart_button button alt" value="<?php echo apply_filters('single_add_to_cart_text', __('', 'woocommerce'), $product->product_type); echo implode('', $value['attributes']);?>"><?php echo apply_filters('single_add_to_cart_text', __('', 'woocommerce'), $product->product_type); echo implode('', $value['attributes']);?></button></li>
		<?php
		}
		echo '</ul>';
		...
		$date = strtolower(date('d-M-Y-hi-a', strtotime($_GET['date']))); ?>
		<script>
...
woocommerce_get_template( 'single-product/tabs/description.php' ); //Event details are commented out for now, until I can style this. echo do_shortcode("[event_details]"); 
echo '</div> </div> </div>';
}
